/**
 * Racket Bible string modulator shell — markup matches legacy `comp-modulator-shell` (Zero-Pixel).
 * SearchableSelect mounts into #comp-mains-select / #comp-crosses-select from compendium.ts after render.
 */
    ...
    <div className="bg-transparent border border-dc-storm/30 p-5 md:p-6 mb-10 flex flex-col gap-5">
      <div className="flex justify-between items-center border-b border-dc-storm/30 pb-3 mb-1">
        ...
        <div className="flex gap-4">
          <button
            type="button"
            className="comp-inject-mode-btn text-dc-accent border-dc-accent border-b-2 pb-1 font-mono text-[12px] uppercase tracking-widest hover:text-dc-platinum transition-colors"
            data-mode="fullbed"
          ...
          <button
            type="button"
            className="comp-inject-mode-btn text-dc-storm border-transparent border-b-2 pb-1 font-mono text-[12px] uppercase tracking-widest hover:text-dc-platinum transition-colors"
            data-mode="hybrid"
            data-comp-action="setInjectMode"
          >
            Hybrid
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
        <div className="flex flex-col gap-3" id="comp-mains-col">
          <span className="font-mono text-[13px] text-dc-storm uppercase tracking-[0.2em]" id="comp-mains-label">
            // STRING
          </span>
          <div id="comp-mains-select" className="comp-string-select-container" />
        ...
        <div className="flex flex-col gap-3" id="comp-crosses-col">
          <span className="font-mono text-[13px] text-dc-storm uppercase tracking-[0.2em]" id="comp-crosses-label">
            // CROSSES
          </span>
          <div id="comp-crosses-select" className="comp-string-select-container" style={{ display: 'none' }} />
          <div className="grid grid-cols-2 gap-4">
            <select
              className="appearance-none bg-transparent border-b border-dc-storm/50 text-dc-platinum font-mono text-sm py-2 outline-none focus:border-dc-accent transition-colors bg-[url('data:image/svg+xml;charset=US-ASCII,%3Csvg%20xmlns=%27http://www.w3.org/2000/svg%27%20width=%2712%27%20height=%2712%27%20viewBox=%270%200%2024%2024%27%20fill=%27none%27%20stroke=%27%235E666C%27%20stroke-width=%272%27%20stroke-linecap=%27round%27%20stroke-linejoin=%27round%27%3E%3Cpolyline%20points=%276%209%2012%2015%2018%209%27%3E%3C/polyline%3E%3C/svg%3E')] bg-no-repeat bg-right pr-5 cursor-pointer"
