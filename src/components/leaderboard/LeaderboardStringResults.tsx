import type { LbStringRowVm } from './leaderboard-results-vm.js';

export type LeaderboardStringResultsProps = {
  rows: LbStringRowVm[];
...
function StringMaterialTag({ material }: { material: string }) {
  ...
  if (m.includes('natural gut')) {
    return (
      <span className="font-mono text-[8px] font-bold px-1.5 py-0.5 border border-amber-500/30 text-amber-400 bg-amber-400/5">GUT</span>
    );
  }
  if (m.includes('multifilament')) {
    return (
      <span className="font-mono text-[8px] font-bold px-1.5 py-0.5 border border-sky-500/30 text-sky-400 bg-sky-400/5">MULTI</span>
    );
  }
  if (m.includes('co-polyester')) {
    return (
      <span className="font-mono text-[8px] font-bold px-1.5 py-0.5 border border-purple-500/25 text-purple-400 bg-purple-400/5">CO-POLY</span>
    );
  }
  if (m.includes('synthetic')) {
    return <span className="font-mono text-[8px] font-bold px-1.5 py-0.5 border border-dc-storm/30 text-dc-storm">SYN GUT</span>;
  }
  return <span className="font-mono text-[8px] font-bold px-1.5 py-0.5 border border-dc-storm/30 text-dc-storm">POLY</span>;
}

export function LeaderboardStringResults({
  rows,
        ...
        <table className="w-full border-collapse">
          <thead>
            <tr className="border-b border-dc-storm/20">
              ...
              <th className="px-3 py-2.5 text-left font-mono text-[8px] font-bold uppercase tracking-[0.15em] text-dc-storm/60">String</th>
              ...
              >
                ...
                <td className="px-3 py-3 min-w-[180px]">
                  <div className="flex items-center gap-2">
                    <span className="font-sans text-[12px] font-semibold text-dc-platinum leading-tight">{entry.stringName}</span>
                    <StringMaterialTag material={entry.material} />
                  </div>
