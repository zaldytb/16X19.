...
export function MyLoadoutsList({ model }: Props) {
    ...
    <>
      {model.rows.map((row) => (
        <div
          ...
          data-lo-id={row.id}
        >
          <div className="flex items-center gap-2.5 flex-1 min-w-0 px-3 py-2.5 cursor-pointer" data-lo-action="switchToLoadout" data-id={row.id}>
          ...
          {row.isConfirmingRemoval ? (
            <div className="flex items-stretch border-l border-[var(--dc-border)]" style={{ opacity: 1, pointerEvents: 'auto' }}>
              ...
              <button className="px-2 font-mono text-[9px] font-bold text-[var(--dc-red)] hover:bg-[var(--dc-void)] border-l border-[var(--dc-border)] h-full transition-colors" data-lo-action="removeLoadout" data-id={row.id}>Yes</button>
              <button className="px-2 font-mono text-[9px] text-[var(--dc-storm)] hover:text-[var(--dc-platinum)] border-l border-[var(--dc-border)] h-full transition-colors" data-lo-action="renderMyLoadouts">No</button>
            </div>
          ) : (
            <div className="flex items-stretch opacity-0 group-hover:opacity-100 transition-opacity border-l border-[var(--dc-border)]">
              <button className="w-8 flex items-center justify-center text-[var(--dc-storm)] hover:text-[var(--dc-platinum)] hover:bg-[var(--dc-void)] transition-colors" data-lo-action="shareLoadout" data-id={row.id} title="Share">
              ...
              <button className="w-8 flex items-center justify-center text-[var(--dc-storm)] hover:text-[var(--dc-platinum)] hover:bg-[var(--dc-void)] transition-colors border-l border-[var(--dc-border)]" data-lo-action="addLoadoutToCompare" data-id={row.id} title="Compare">
              ...
              <button className="w-8 flex items-center justify-center text-[var(--dc-storm)] hover:text-[var(--dc-red)] hover:bg-[var(--dc-void)] transition-colors border-l border-[var(--dc-border)]" data-lo-action="confirmRemoveLoadout" data-id={row.id} title="Remove">
